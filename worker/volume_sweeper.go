...
func (sweeper *volumeSweeper) sweep(logger lager.Logger) {
	...
	volumes, err := sweeper.baggageclaimClient.ListVolumes(context.TODO(), baggageclaim.VolumeProperties{})
	if err != nil {
	...
	if err != nil {
		...
		for _, handle := range volumeHandles {
			...
			go func(handle string) {
				err := sweeper.baggageclaimClient.DestroyVolume(context.TODO(), handle)
				if err != nil {
